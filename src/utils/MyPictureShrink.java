...
public class MyPictureShrink {
	public static File createThumbnail(File srcImage) {
		final int imageSize = 100;
		File thumbnail = new File(srcImage.getParent() + "/thumb/" + srcImage.getName());
		...
		try {
			...
			BufferedImage img2;
			if (width > height) {
				float scaledWidth = (width / height) * imageSize;
				...
				float scaledHeight = (height / width) * imageSize;
			...
			ImageIO.write(img2, "jpg", thumbnail);
		} catch (IOException e) {
